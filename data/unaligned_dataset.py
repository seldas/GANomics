...
from data.image_folder import make_dataset as make_image_dataset
...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.datatype == 'image':
            ...
            self.A_paths = sorted(make_image_dataset(self.dir_A, opt.max_dataset_size))   # load images from '/path/to/data/trainA'
            self.B_paths = sorted(make_image_dataset(self.dir_B, opt.max_dataset_size))    # load images from '/path/to/data/trainB'
    ...
    def __getitem__(self, index):
        ...
        Returns a dictionary that contains A, B, A_paths and B_paths
            A (tensor)       -- an image or text in the input domain
            B (tensor)       -- its corresponding image or text in the target domain
            A_paths (str)    -- image or text paths
            B_paths (str)    -- image or text paths
        """
        ...
        if self.opt.datatype == 'text':
            A = text_default_loader(A_path, True)
            B = text_default_loader(B_path, True)
