...
const LossModal = ({ runId, onClose }: { runId: string; onClose: () => void }) => {
  const [data, setData] = useState<LossPoint[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    axios
      .get(`${API_BASE}/manuscript/logs/${runId}`)
  ...
  const displayedData = useMemo(() => {
    ...
    const smoothed = data.map((point, index) => {
      ...
      const avg = (key: string) =>
        subset.reduce((sum, p) => sum + (Number(p[key]) || 0), 0) / subset.length;
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: '80%', maxWidth: '900px', height: '600px', display: 'flex', flexDirection: 'column' }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0 }}>Validation Losses: {runId}</h3>
          <button onClick={onClose} className="chip" style={{ padding: '4px' }}>
            <X size={20} />
          </button>
        </div>
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          {loading ? (
            ...
            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              No loss data available in logs.
            </div>
          )}
        </div>
      </div>
...
const TaskTable = ({
  ...
  const StatusIcon = ({ exists }: { exists: boolean }) =>
    exists ? <CheckCircle2 size={16} color="#16a34a" /> : <Clock size={16} color="#94a3b8" />;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <tr key={task.run_id} className="hover-row" style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <span
                      ...
                      style={{
                        fontSize: '0.7rem',
                        backgroundColor: task.dataset === 'CycleGAN' ? '#fef3c7' : 'var(--bg-muted)',
                        color: task.dataset === 'CycleGAN' ? '#92400e' : 'inherit'
                      }}
                    >
                      {task.dataset}
                    </span>
                  ...
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <StatusIcon exists={task.status.sync} />
                  </td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <StatusIcon exists={task.status.comparative} />
                  </td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <StatusIcon exists={task.status.deg} />
                  </td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <StatusIcon exists={task.status.pathway} />
                  </td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <StatusIcon exists={task.status.prediction} />
                  </td>
      ...
      {selectedRunId && <LossModal runId={selectedRunId} onClose={() => setSelectedRunId(null)} />}
    </div>
  );
};
...
export const ManuscriptRecords: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await axios.get<ManuscriptTask[]>(`${API_BASE}/manuscript/tasks`);
        setTasks(res.data ?? []);
      } catch (err) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div className="card" style={{ padding: '1.5rem 2rem' }}>
        ...
        <p style={{ margin: '0.5rem 0 0 0', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
          Displaying archived manuscript experiments from the database.
        </p>
      </div>

      <TaskTable title="Neuroblastoma (NB & CycleGAN)" tasks={nbGroup} icon={LayoutGrid} />

      <TaskTable title="Generalization (Other Projects)" tasks={otherGroup} icon={Globe} />
